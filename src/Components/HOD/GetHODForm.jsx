...
function GetHODForm() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      try {
        const response = await fetch(`${host}/api/data/hod/`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            authtoken: localStorage.getItem("authtoken_hod")
          },
        });
  ...
  const handleAccept = async (_id) => {
    try {
      const response = await fetch(`${host}/api/data/hod/accept`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authtoken: localStorage.getItem("authtoken_hod")
  ...
  const handleReject = async (_id, parentMessage) => {
    try {
      const response = await fetch(`${host}/api/data/hod/decline`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authtoken: localStorage.getItem("authtoken_hod")
  ...
  return (
    <div className="container mt-5">
      <h2 className="mb-4">HOD Forms</h2>
      <div className="row">
        {forms.map((form) => (
          <div className="col-md-4 mb-4" key={form._id}>
            <div className="card">
              <div className="card-body">
                ...
                <p className="card-text">Hod Accepted: {form.teacher_accepted.toString()}</p>
                <p className="card-text">Hod Rejected: {form.teacher_rejected.toString()}</p>
                <p className="card-text">Hod Message: {form.teacher_message}</p>
                <div className="mt-3">
