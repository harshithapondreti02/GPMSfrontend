...
function GetParentForm() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      try {
        const response = await fetch(`${host}/api/data/parent/`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            authtoken: localStorage.getItem("authtoken_parent")
          },
        });
  ...
  const handleAccept = async (_id) => {
    try {
      const response = await fetch(`${host}/api/data/parent/accept`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authtoken: localStorage.getItem("authtoken_parent")
  ...
  const handleReject = async (_id, parentMessage) => {
    try {
      const response = await fetch(`${host}/api/data/parent/decline`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authtoken: localStorage.getItem("authtoken_parent")
        },
        body: JSON.stringify({ _id, parent_message: parentMessage }),
      });
  ...
  return (
    <div className="container mt-5">
      <h2 className="mb-4">Parent Forms</h2>
      <div className="row">
        {forms.map((form) => (
